...
num_cols = 2  # Display each image and its thresholded version side by side
...
for idx, file in enumerate(files):
    ...
    axes[row_idx, col_idx * 2].imshow(img, cmap='gray')
    axes[row_idx, col_idx * 2].set_title(f'Time = {time} hr\nOriginal Image')

    # Display the thresholded image in the right subplot with a different color
    axes[row_idx, col_idx * 2 + 1].imshow(binary, cmap='coolwarm', interpolation='none')
    axes[row_idx, col_idx * 2 + 1].set_title('Thresholded Image')
